...
SERVICE_NAME=drc-performance-test
...
then
    ...
    MAX_DIRECT=2
    #JAVA_OPTS="$JAVA_OPTS -ea -Xms${USED_MEM}g -Xmx${USED_MEM}g -Xmn${XMN}g -XX:+AlwaysPreTouch  -XX:MaxDirectMemorySize=${MAX_DIRECT}g "
    JAVA_OPTS="$JAVA_OPTS -ea -Xms${USED_MEM}g -Xmx${USED_MEM}g -XX:+AlwaysPreTouch  -XX:MaxDirectMemorySize=${MAX_DIRECT}g -Ddrc.applier.min.directmemory=1000000"
else
    #MB
    USED_MEM=1600
    XMN=600
