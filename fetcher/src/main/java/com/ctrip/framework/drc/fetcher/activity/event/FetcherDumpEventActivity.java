 ...
 * 2024/10/28 14:38
 */
public abstract class FetcherDumpEventActivity extends DumpEventActivity<FetcherEvent> {
    ...
    protected String lastReceivedUuid;

    protected boolean gapInited = false;

    protected GtidSet.Interval receivedStartAndEndTrxId;

    public GtidSet toInitGap = new GtidSet(StringUtils.EMPTY);

    protected GtidSet previousGtidSet = new GtidSet(StringUtils.EMPTY);

    protected Set<String> uuids = Sets.newHashSet();

    //key: uuid, value: last received trxId
    protected Map<String, Long> lastReceivedTxs = Maps.newHashMap();


    @Override
    public void doHandleLogEvent(FetcherEvent event) {
    ...
    protected void persistPosition(GtidSet gtidSet) {
     //不删除因为compensateGap里用到，在TransactionTableApplierDumpEventActivity和MqApplierDumpEventActivity里分别重写了
    }
    ...
    public void updateContextGtidSet(GtidSet gtidset) {
    ...
    public void updateContextGtidSet(Gtid gtid) {
        GtidSet set = context.fetchGtidSet();
        set.add(gtid);
    }
