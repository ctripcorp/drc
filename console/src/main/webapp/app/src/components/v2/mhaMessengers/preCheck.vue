<template v-if="current === 0" :key="0">
  <div>
    <Form  :model="mhaInfo"  :label-width="250" style="float: left; margin-top: 50px">
          <FormItem label="源集群名" prop="mhaName" style="width: 600px">
            <Row>
              <Col span="16">
                <Input v-model="mhaInfo.mhaName"  readonly placeholder="请输入源集群名"/>
              </Col>
              <Col span="5">
                <Button  type="primary" @click="checkMySqlConfig(mhaInfo)" ghost style="margin-left: 10px">配置校验</Button>
              </Col>
            </Row>
          </FormItem>
          <FormItem label="同步表" prop="ip">
            <Row>
              <Col span="16">
                <Input v-model="mhaInfo.nameFilter"   placeholder="支持正则，默认全部"/>
              </Col>
              <Col span="5">
                <Button  type="primary" @click="checkMySqlTables(mhaInfo)" ghost style="margin-left: 10px">表校验</Button>
              </Col>
            </Row>
          </FormItem>
        </Form>
...
export default {
  ...
  methods: {
    ...
    checkMySqlTables (mha) {
      ...
      this.axios.get('/api/drc/v1/build/preCheckMySqlTables?mha=' + this.mhaInfo.mhaName +
        '&' + 'nameFilter=' + this.mhaInfo.nameFilter)
        .then(response => {
