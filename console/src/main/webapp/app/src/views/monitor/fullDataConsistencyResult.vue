<template>
  <base-component :isFather="isFather" :subMenuName="['1']" :fatherMenu="fatherMenu">
    <Breadcrumb :style="{margin: '15px 0 15px 185px', position: 'fixed'}">
      ...
      <BreadcrumbItem to="/fullDataConsistencyCheck">校验表配置</BreadcrumbItem>
    ...
    <Content class="content" :style="{padding: '10px', background: '#fff', margin: '50px 0 1px 185px', zIndex: '1'}">
      <div style="padding: 1px 1px">
        <Card>
          ...
          <div class="ivu-list-item-meta-title">
            <p>数据库名：{{this.dbName}}</p>
          </div>
          <div class="ivu-list-item-meta-title">
            <p>表名：{{this.tableName}}</p>
          </div>
...
export default {
  ...
  methods: {
    getCurrentRecord () {
      const that = this
      this.axios.get('api/drc/v1/monitor/consistency/full/history/' + this.$route.query.mhaA + '/' + this.$route.query.mhaB + '/' + this.$route.query.dbName + '/' + this.$route.query.tableName + '/' + this.$route.query.key + '/' + this.$route.query.checkTime)
        .then(res => {
          ...
          const schema = this.$route.query.tableName.split('.')
          this.dbName = schema[0]
          this.tableName = schema[1]
        })
    },
    SubmitSrcSql () {
      setTimeout(() => {
