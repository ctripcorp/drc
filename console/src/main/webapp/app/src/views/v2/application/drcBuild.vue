<template>
  <base-component>
    ...
    <Content class="content"
      ...
      <Collapse>
        <Panel>
          mha配置预检测：<Tag  :loading="mysqlConfig.dataLoading" :type="mysqlConfig.result?'success':'volcano'" @click="preCheckMysqlConfig">{{ mysqlConfig.resultStr }}</Tag>
          <template #content>
            <Table border stripe :loading="mysqlConfig.dataLoading" :columns="mysqlConfig.columns" :data="mysqlConfig.data"></Table>
          </template>
        </Panel>
      </Collapse>
      ...
      >
        ...
        <div class="drawer-footer">
          ...
          <Button v-show="!successSubmit" type="primary" @click="submitAll" :loading="dataLoading">审批通过</Button>
        </div>
      </Drawer>
    </Content>
  </base-component>
...
export default {
    ...
    return {
      ...
      mysqlConfig: {
        ...
        columns: [
          ...
          {
            title: 'BINLOG_VERSION [OFF]',
            key: 'binlogVersion1'
          },
          {
            title: 'BTD [WRITESET]',
            key: 'binlogTransactionDependency'
          },
          {
            title: 'BTDHS [100000]',
            key: 'binlogTransactionDependencyHistorySize'
          },
          {
            title: 'BINLOG_ROW_IMAGE [FULL]',
            key: 'binlogRowImage'
          },
          {
            title: 'GTID_MODE [ON]',
          ...
          {
            title: 'AUTO_INCREMENT_STEP',
            key: 'autoIncrementStep'
          },
          {
            title: 'AUTO_INCREMENT_OFFSET',
            key: 'autoIncrementOffset'
          },
          {
            title: 'DRC_ACCOUNT [three accounts ready]',
            key: 'drcAccounts'
          },
          {
            title: '结果',
            ...
            render: (h, params) => {
              ...
              const color = row.result ? 'green' : 'volcano'
              const text = row.result ? '合格' : '不合格'
              return h('Tag', {
                props: {
                  color: color
                }
  ...
  methods: {
    ...
    getParams: function () {
      const param = {}
      param.applicationFormId = this.applicationFormId
    ...
    async submitAll () {
      ...
      console.log(params)
      await that.axios.post('/api/drc/v2/autoconfig/applicationForm/submit', params)
        .then(response => {
    ...
    preCheckMysqlConfig () {
      this.mysqlConfig.dataLoading = true
      const mhaList = []
      console.log('previewDataList:')
      console.log(this.previewDataList)
      this.previewDataList.forEach(data => {
      ...
      this.axios.get('/api/drc/v2/autoconfig/preCheckMysqlConfig?mhaList=' + mhaList).then(res => {
        if (res.data.status === 0) {
          ...
          this.mysqlConfig.data = data.configs
          this.mysqlConfig.result = data.result
          if (data.result) {
            this.mysqlConfig.resultStr = '合格'
          } else {
            this.mysqlConfig.resultStr = '不合格'
          }
        } else {
          this.$Message.warning('mha配置校验失败: ' + res.data.message)
        }
      })
      this.mysqlConfig.dataLoading = false
    ...
    init () {
      ...
      this.formItem.tableName = this.$route.query.tableName
