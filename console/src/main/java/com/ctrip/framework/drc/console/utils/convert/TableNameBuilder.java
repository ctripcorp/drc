...
public class TableNameBuilder {
    ...
    public static String buildNameFilter(Map<Long, String> srcMappingIdToDbNameMap, List<DbReplicationTbl> dbReplicationTblList) {
        List<String> nameFilterList = new ArrayList<>();
        for (DbReplicationTbl dbReplicationTbl : dbReplicationTblList) {
            String dbName = srcMappingIdToDbNameMap.get(dbReplicationTbl.getSrcMhaDbMappingId());
            if (!StringUtils.isBlank(dbName)) {
                nameFilterList.add(dbName + "\\." + dbReplicationTbl.getSrcLogicTableName());
            }
        }
        return Joiner.on(",").join(nameFilterList);
    }

    public static String buildNameMapping(Map<Long, String> mappingIdToDbTblMap, List<DbReplicationTbl> dbReplicationTblList) {
        List<String> nameMappingList = new ArrayList<>();
        for (DbReplicationTbl dbReplicationTbl : dbReplicationTblList) {
            if (StringUtils.isBlank(dbReplicationTbl.getDstLogicTableName())) {
                continue;
            }
            String srcDbName = Optional.ofNullable(mappingIdToDbTblMap.get(dbReplicationTbl.getSrcMhaDbMappingId())).orElse("");
            String dstDbName = Optional.ofNullable(mappingIdToDbTblMap.get(dbReplicationTbl.getDstMhaDbMappingId())).orElse("");
            nameMappingList.add(srcDbName + "." + dbReplicationTbl.getSrcLogicTableName() + "," + dstDbName + "." + dbReplicationTbl.getDstLogicTableName());
        }
        if (CollectionUtils.isEmpty(nameMappingList)) {
            return null;
