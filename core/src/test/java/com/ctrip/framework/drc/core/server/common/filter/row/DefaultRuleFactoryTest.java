...
public class DefaultRuleFactoryTest {
    ...
    public void createRowsFilterRule() throws Exception {
        ...
        properties = String.format(ROW_FILTER_PROPERTIES, RowsFilterType.None.getName(), location);
        ...
        properties = String.format(ROW_FILTER_PROPERTIES, RowsFilterType.AviatorRegex.getName(), location);
        dataMediaConfig = DataMediaConfig.from(registryKey, properties);
        rowsFilterRule = ruleFactory.createRowsFilterRule(dataMediaConfig.getRowsFilters().get(0));
        Assert.assertTrue(rowsFilterRule instanceof AviatorRegexRowsFilterRule);

        properties = String.format(ROW_FILTER_PROPERTIES, RowsFilterType.JavaRegex.getName(), location);
        dataMediaConfig = DataMediaConfig.from(registryKey, properties);
        rowsFilterRule = ruleFactory.createRowsFilterRule(dataMediaConfig.getRowsFilters().get(0));
        ...
        properties = String.format(ROW_FILTER_PROPERTIES, RowsFilterType.Custom.getName(), location);
        dataMediaConfig = DataMediaConfig.from(registryKey, properties);
        rowsFilterRule = ruleFactory.createRowsFilterRule(dataMediaConfig.getRowsFilters().get(0));
